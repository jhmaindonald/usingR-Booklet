...
## `plot()` and allied functions -- some further details
...
### Multiple plots on the one page {.unnumbered}
...
Use the function `points()` to add points. Use `lines()` to join
successive lines to points.
...
It sets `pch=16` to make the plot character a heavy black dot.  This helps make the points stand out against the labeling.  
