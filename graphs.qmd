...
from a normal distribution.  @fig-fig12 is an example.

```{r, echo=F}
cap12 <- "Normal probability plots. If data are from a normal distribution 
...
By default, `rnorm()` generates random samples from a distribution with mean 0 
...
```{r, echo=F}
cap19 <- "Plot of ``hc`` against ``rcc``. Basketballers are compared with
...
`
```{r, echo=F}
cap20 <- "Examples where one or both axis labels are mathematical 
expressions,"
```

```{r, fig.width=7.5, fig.asp=0.55, out.width='100%', echo=F}
#| label: fig-fig20
#| fig-cap: !expr paste(cap20)
